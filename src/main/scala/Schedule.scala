...
object Schedule {
  ...
  val cleaningTimesSeconds = Source.fromResource("BP_times.csv").getLines.map { line =>
  ...
  val cleaningBunkerConsumptionsMr = Source.fromResource("mr_cleaning_bunker_consumption.csv").getLines.drop(1).map { line =>
  ...
  val cleaningBunkerConsumptionsLr = Source.fromResource("lr_cleaning_bunker_consumption.csv").getLines.drop(1).map { line =>
    val split = line.split(",", 4)
    val from = split(0).toLowerCase
    val to = split(1).toLowerCase
    val fuel = split(2).toDouble
  ...
  case class ActionCost(cost: Double, startDate: Long, duration: Long, distanceTravelled: Double, fuelConsumed: Double, locationAfter: Schema.Port, constraintsSatisfied: Boolean) extends Cost

  case class PrepareAndTravelCost(cost: Double, startDate: Long, duration: Long, travelSpeed: Double, travelDuration: Long, distanceTravelled: Double, fuelConsumed: Double, locationAfter: Schema.Port, constraintsSatisfied: Boolean, cleaningAction: CleaningAction, refuelingAction: RefuelingAction, suezCost: Double) extends Cost
  ...
  object AnnotatedRequirement {
    def apply(requirement: Requirement, vessel: Schema.VesselDimensions, contractExpiry: Long, portRestrictions: Schema.PortIncompatabilityMap, unavailableTimes: Seq[UnavailableTime], location: Schema.Port, startDate: Long, remainingFuel: Double, currentProduct: Schema.Product)(implicit context: Context): AnnotatedRequirement =
      new AnnotatedRequirement(requirement = requirement, requirement.cost(vessel, contractExpiry, portRestrictions, unavailableTimes, startDate, remainingFuel, location, currentProduct))
  }


  // Captures additional cost of a refueling action if one must take place.
  case class RefuelingAction(duration: Long, cost: Double, fuel: Double, port: Schema.Port, travel1Duration: Long, travel2Duration: Long, fuel1: Double, fuel2: Double, refuelingDuration: Long)

  object RefuelingAction {
    def refuel(vessel: Schema.VesselDimensions, portRestrictions: Schema.PortIncompatabilityMap,now: Long, speed: Double, fuelConsumedPerDay: Double, distanceToTarget: Double, durationToTarget: Long, currentFuel: Double, fuelToTarget: Double, at: Schema.Port, target: Schema.Port)(implicit context: Context): Option[RefuelingAction] = {
      ...
      if (distanceToTarget == 0 || !fuelLow(fuelAtTarget)) {
        ...
        val unsortedPorts = at.neighbors
          .filter { p => Schema.dockingFeasable(vessel, p) } // Must be able to enter the port
          .map { n => n -> (distanceNm(at.id, n.id).value, distanceNm(n.id, target.id).value) }.filter(_._2._1 < maxRange) // Must be within range
          .filter { // Port incompatabilities - this isn't quite right, vessel could wait for port incompatibility to expire
            case (p, (d1, d2)) =>
              ...
              val notRestricted = invalidRanges.forall(r => !DateRange(arrival, arrival + secondsPerDay.toLong).overlaps(r.dateRange))
              notRestricted
          }
        ...
        if (unsortedPorts.isEmpty) {
          ...
          if(bestPort.id == target.id){
            ...
            Some(RefuelingAction(0, 0, 0, target, 0, 0, 0, 0, 0))
             ...
             println(s"Refueling Vessel: ${vessel.id} - Using port ${bestPort.id}, distance from source to dest = ${distanceNm(at.id, target.id)}, broken to $d1 -> $d2")
  ...
  case class CleaningAction(totalCost: Double, durationCost: Double, fuelCost: Double, dieselCost: Double, fuelConsumed: Double, duration: Long, nextProduct: Schema.Product)

  object CleaningAction {
    def clean(vessel: Schema.VesselDimensions, currentProduct: Schema.Product, requirementProduct: Schema.Product, productQuantityM3: Double): CleaningAction = {
  ...
  def calculateSuezCost(scnt: Double, ballast: Boolean, productType: String): Double = {
    ...
    val suezRatesPerSCNT = if (ballast) List(6.70, 4.74, 3.59, 1.78, 1.53, 1.32, 1.29) else {
      productType match {
    ...
    val suezCost = quantities.zip(suezRatesPerSCNT).foldLeft((0.0, scnt))( {
  ...
  def prepareAndTravelCost(
    ...
    val suezCost = distance.suezRoute match {
      case true => println(s"calculating suez cost for vessel: ${vessel.name}"); calculateSuezCost(vessel.scnt, ballast, vessel.cleanStatus)
      case false => 0.0
    }
    ...
    val refuelingActionO = RefuelingAction.refuel(vessel, portRestrictions, startDate, speed, fuelConsumedPerDay, distance.value, travelDuration, startFuel, fuelConsumed, currentPort, dest)
    refuelingActionO.map { refuelingAction =>
      ...
      PrepareAndTravelCost(totalCost, startDate, totalDuration, speed, travelDuration, distance.value, totalFuelConsumed, dest, true, cleaningAction, refuelingAction, suezCost)
    }.getOrElse {
      ...
      PrepareAndTravelCost(0, startDate, 0, 0, 10000000, 0, 0, dest, false, cleaningAction, RefuelingAction(0, 0, 0, dest, 0, 0, 0, 0, 0), suezCost)
  ...
  case class PrepareAndTravelTo(dest: Schema.Port, completeAfter: Long, startBefore: Long) extends VesselAction {
    def cost(vessel: Schema.VesselDimensions, portRestrictions: Schema.PortIncompatabilityMap, startDate: Long, startFuel: Double, currentPort: Schema.Port, currentProduct: Schema.Product, startProduct: Schema.Product, productQuantityM3: Double)(implicit context: Context): PrepareAndTravelCost = {
      ...
      prepareAndTravelCost(vessel, portRestrictions, startDate, startFuel, currentPort, distance, vessel.ballastEconomicSpeed, vessel.ballastMaxSpeed, vessel.ballastBunkerRequirmentsMtPerDay, completeAfter, startBefore, dest, cleaningAction, true)
    }
  }

  // Distance to travel between the Ports - when known as a part of a requirements - cached distance in effect
  case class PrepareAndTravelDist(dest: Schema.Port, distance: Queries.PortDistance, completeAfter: Long, startBefore: Long) extends VesselAction {
    // Subsequent Voyages always laden
    def cost(vessel: Schema.VesselDimensions, portRestrictions: Schema.PortIncompatabilityMap, startDate: Long, startFuel: Double, currentPort: Schema.Port)(implicit context: Context): PrepareAndTravelCost = {
      // only clean at start of Requirement - other case
      prepareAndTravelCost(vessel, portRestrictions, startDate, startFuel, currentPort, distance, vessel.ladenEconomicSpeed, vessel.ladenMaxSpeed, vessel.ladenBunkerRequirmentsMtPerDay, completeAfter, startBefore, dest, CleaningAction(0, 0, 0, 0, 0, 0, Schema.Product("", "")), false)
    }
  }

  // Corresponds to a Long
  case class Load(duration: Long, acost: Double, product: Schema.Product, port: Schema.Port, valid: DateRange) extends VesselAction {
  ...
  case class Unload(duration: Long, acost: Double, product: Schema.Product, port: Schema.Port, valid: DateRange) extends VesselAction {
    def cost(vessel: Schema.VesselDimensions, startDate: Long): ActionCost = {
      val constraint = valid.contains(startDate)
      ActionCost(acost, startDate, duration, 0, 0, port, constraint)
    }
  ...
  case class Requirement(id: Int, initialProduct: Schema.Product, finalProduct: Schema.Product, actions: Seq[VesselAction], originalRequirement: Schema.Requirement) extends VesselAction {
    def costInner(vessel: Schema.VesselDimensions, contractExpiry: Long, portRestrictions: Schema.PortIncompatabilityMap, unavailableTimes: Seq[UnavailableTime], startDate: Long, startFuel: Double, currentPort: Schema.Port, currentProduct: Schema.Product)(implicit context: Context): ActionCost = {
      // cost is sum of costs of actions
      case class VState(now: Long, currentFuel: Double)

      val initial: (VState, ActionCost) = (VState(startDate, startFuel), ActionCost(0, startDate, 0, 0, 0, currentPort, true))
      val timeWithCost = actions.foldLeft(initial) { case ((vs, c), a) =>
        val cost = a match {
          ...
          case wt: PrepareAndTravelTo => wt.cost(vessel, portRestrictions, vs.now, vs.currentFuel, c.locationAfter, currentProduct, this.initialProduct, this.originalRequirement.firstAction.productQuantityM3) // Only deal with currently deal with cleaning at the start of a Requirement
          case r: Requirement => r.cost(vessel, contractExpiry, portRestrictions, unavailableTimes, vs.now, vs.currentFuel, c.locationAfter, this.initialProduct) // shouldn't happen but if it's nested the product is assumed to be the same as we clean for
          case ar: AnnotatedRequirement => ar.cost
        }

        (VState(cost.startDate + cost.duration, vs.currentFuel - cost.fuelConsumed),
          ActionCost(c.cost + cost.cost,
            startDate,
    ...
    def cost(vessel: Schema.VesselDimensions, contractExpiry: Long, portRestrictions: Schema.PortIncompatabilityMap, unavailableTimes: Seq[UnavailableTime], startDate: Long, startFuel: Double, currentPort: Schema.Port, currentProduct: Schema.Product)(implicit context: Context): ActionCost = {
      val possibleCost = costInner(vessel, contractExpiry, portRestrictions, unavailableTimes, startDate, startFuel, currentPort, currentProduct)
  ...
  object Requirement {
    def apply(id: Int, requirement: Schema.Requirement, vessel: Schema.VesselDimensions)(implicit context: Context): Requirement = {
      // currently all Longs must complete before all Shorts
      val portActions: Seq[Schema.PortAction] = requirement.longs.sortBy(_.valid.startDate) ++ requirement.shorts.sortBy(_.valid.startDate)
      val withPorts = portActions.map { a => a -> context.portMap(a.portId) }
      ...
      val otherActions: Seq[VesselAction] = remainingPortActions.flatMap { case (action, port, prevPort) =>
        ...
        List(
          ...
          action match {
            case a@Schema.LongAction(product, quantity, valid, ogDateRange, portId, terminalId) => Load(port, a, vessel)
            case a@Schema.ShortAction(product, quantity, valid, ogDateRange, portId, terminalId) => Unload(port, a, vessel)
          }
        )
      }
  ...
  case class Context(
                    ...
                    distanceCache: scala.collection.mutable.Map[(String, String), Future[Queries.PortDistance]] = new java.util.concurrent.ConcurrentHashMap[(String, String), Future[Queries.PortDistance]].asScala
                    )
  ...
  def distanceNm(from: PortId, to: PortId)(implicit context: Context): Queries.PortDistance = {
    if (from == to) {
      ...
      val future = context.distanceCache.getOrElse((from, to),
  ...
  */
  def possibleSchedules(state: State)(implicit context: Context): List[Schedule] = {
    ...
    val distances = state.candidateRequirements.map { r => r -> distanceNm(state.port.id, r.firstAction.portId).value }
    ...
    val requirements = canReach.map { case (r, _, _) => AnnotatedRequirement(r, state.vessel, state.contract.expiration, state.portRestrictions, state.unavailableTimes, state.port, state.now, state.currentFuel, state.currentProduct) }
  ...
  def schedule(now: Long, vessel: Schema.VesselWithDimensions, requirementsToSchedule: Seq[Schema.Requirement], context: Context) = {
    ...
    val patchedRequirements = requirementsToSchedule.map {r =>
      if (r.locked.isDefined) {
        ...
        val newLongs = r.longs.zip(longStarts).map { case (l, newStart) =>
          l.copy(valid = DateRange(newStart - 1, Math.max(newStart + 1 * secondsPerHour.toLong, l.valid.endDate + (secondsPerDay*1.5).toLong)))
        }
        val newShorts = r.shorts.zip(shortStarts).map { case (s, newStart) =>
          s.copy(valid = DateRange(newStart - 1, Math.max(newStart + 1 * secondsPerHour.toLong, s.valid.endDate + (secondsPerDay*1.5).toLong)))
        }
    ...
    val res = res0.flatMap {s => filterByRequired(s, requiredReqs)}
    ...
    val maxdepth = res.map(scheduleMaxDepth).fold(0L) { (a, b) => Math.max(a, b) }
...
//    println
  ...
  def walkSchedules[T](fn: Seq[AnnotatedRequirement] => T, schedule: Schedule, acc: Seq[AnnotatedRequirement] = Seq.empty): Seq[T] = schedule.children match {
  ...
  def arToString(ar: AnnotatedRequirement): String = {
    def startDate0 = new DateTime(ar.requirement.firstAction.valid.startDate * 1000).toDateTime.toString(dateFormatString)
  ...
  def printScheduleRequirementEnds(in: Seq[AnnotatedRequirement]) : Unit = {
    val strs = in.map(r => s""""${toDate(r.cost.startDate)} - ${ toDate(r.cost.startDate+r.cost.duration)}"""").mkString(",")
    println(s"[$strs]")
  }
  ...
  def printSchedule(fuel: Double)(in: Seq[AnnotatedRequirement]): Unit = {
    ...
    val endFuels = in.foldLeft(initialFuel) { case ((fuel, endFuels), ar) =>
      val endFuel = fuel - ar.cost.fuelConsumed
      (endFuel, endFuels :+ endFuel)
    }._2

    val outStr = in.zip(endDates.zip(endFuels)).map { case (ar, (ed, ef)) =>
  ...
  def expandScheduleI(state: State, requirements: Seq[Requirement])(implicit context: Context): Seq[ScheduleResults.Requirement] = {

    val vessel = state.vessel
    // Cost the requirements
    val (_, reqs) = requirements.foldLeft((state, Seq.empty[ScheduleResults.Requirement])) { case ((vs1, rs), r) =>
      ...
      val rcost  = r.cost(vessel, state.contract.expiration, vs1.portRestrictions, vs1.unavailableTimes, vs1.now, vs1.currentFuel, vs1.port, vs1.currentProduct)

      // Start port can change because of unavailable time
      val newPort = vs1.unavailableTimes.takeWhile(u => u.dateRange.startDate < rcost.startDate).lastOption.map(u => context.portMap(u.endPort)).getOrElse(vs1.port)

      val (stateAfter, actions) : (State, Seq[ScheduleResults.Action])= r.actions.foldLeft((vs1.copy(now = rcost.startDate, port = newPort), Seq.empty[ScheduleResults.Action])) { case ((vs, as), a) =>
        val (aouts, cost, newProduct) : (Seq[ScheduleResults.Action], Cost, Schema.Product) = a match {
          ...
          case wt: PrepareAndTravelTo =>
            ...
            val cost = wt.cost(vessel, vs.portRestrictions, vs.now, vs.currentFuel, vs.port, vs.currentProduct, r.initialProduct, r.firstAction.productQuantityM3)
  ...
  def expandSchedule(now: Long, vessel: Schema.VesselWithDimensions, requirementsToSchedule: Seq[Schema.Requirement], context: Context): ScheduleResults.DetailedSchedule = {
    val startingPort = vessel.startLocation
    val startingFuel = vessel.startFuel
    // Schedule start occurs after latest of current requirement, ongoing Action or now
    val scheduleStartTime = Math.max(vessel.startDate, now)
